...
function extractBytesFromAgentResponse(response: unknown): string | undefined {
    const agentResponse = response as AgentResponse;
    if (
      agentResponse &&
      agentResponse.intermediateSteps &&
      agentResponse.intermediateSteps.length > 0 &&
      agentResponse.intermediateSteps[0].observation
    ) {
      const obs = agentResponse.intermediateSteps[0].observation;
      try {
